...
const createSubmitter = require('./tx-submitter.js')
...
async function fundChannel ({ api, channel, amount, address, secret }) {
  debug(`funding channel ${channel} for ${amount}`)
  const txSubmitter = createSubmitter(api, address, secret)
  const xrpAmount = dropsToXrp(amount)
  return txSubmitter.submit('preparePaymentChannelFund', {
    amount: xrpAmount,
    channel
  })
}
