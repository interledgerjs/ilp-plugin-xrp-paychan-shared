...
class TxSubmitter {
  constructor (api, address, secret) {
  ...
  _removePendingTx (maxLedgerVersion, txId) {
    ...
    const transactions = this._pendingTx[maxLedgerVersion]
    const txDetails = transactions.find(i => i.txId === txId)
    if (txDetails) {
  ...
  _addTransactionHandler () {
    this._api.connection.on('transaction', (ev) => {
      if (ev.validated && ev.transaction && this._pendingTxPromises[ev.transaction.hash]) {
        ...
        if (ev.meta.TransactionResult !== 'tesSUCCESS') {
    ...
    this._api.connection.on('ledgerClosed', async (ledger) => {
      ...
      for (const v of Object.keys(this._pendingTx).map(x => Number(x))) {
        if (await this._api.connection.hasLedgerVersion(v)) {
          ...
          for (const {txId, minLedgerVersion} of this._pendingTx[v]) {
            const opts = {minLedgerVersion, maxLedgerVersion: v}
            debug(`Obtaining transaction details for ${txId}.`)
            try {
              await this._verifyTx(txId, opts)
            } catch (err) {
              if (err.name === 'MissingLedgerHistoryError') {
                ...
                await this._verifyTx(txId, opts, true)
              }
            }
            this._removePendingTx(v, txId)
          }
        }
      }
    })
  ...
  async submit (fn, ...args) {
    ...
    const {txId, instructions, minLedgerVersion, result} = await new Promise((resolve, reject) => {
      this._txPipeline.then(async () => {
        try {
          ...
          resolve({txId: signed.id, instructions: tx.instructions, minLedgerVersion, result})
        } catch (err) {
          reject(err)
        }
      })
    ...
    if (/^tem/.test(result.resultCode)) {
      debug(`tx malformed. txId=${txId}`)
      throw new Error('Malformed transaction: ' + result.resultMessage)
    }

    this._addPendingTx(instructions.maxLedgerVersion, {txId, minLedgerVersion})

    // resolves if the transaction is included in a validated ledger
    // rejects if the transaction failed with finality
    debug(`waiting for transaction ${txId} to be included in a validated ledger`)
    return new Promise((resolve, reject) => {
      this._pendingTxPromises[txId] = { resolve, reject }
    })
  }
}
...
module.exports = function (_api, _address, _secret) {
  if (!submitterCreated()) {
    const instance = new TxSubmitter(_api, _address, _secret)
    global[sym] = instance
  } else {
